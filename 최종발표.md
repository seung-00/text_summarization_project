...
* 어텐션 메커니즘
  ...
  * 디코더의 time step i에서의 hidden state(query)
  ...
  <img src="https://github.com/hccho2/Tensorflow-2-RNN-Tutorial/raw/master/2.%20Attention-With-Tensorflow/score.png" width="650" height="420">
...
* 계산 과정
  ...
  * score에 softmax 함수를 적용해 합이 1이 되도록 확률값으로 변환
    $$
    attention\ vector\ :\ α_{ij}=\frac{exp(e_{ij})}{\sum_{k=1}^{T_x}exp(e_{ik})}
    $$

  * 디코더 예측에 쓰이는 i번째 context vector 가중합
    $$
    \vec c_i=\sum_{k=1}^{T_x}a_{ij}h_j
    $$
